...
def train(epoch = 100, batch_size = 100, input_size = 2):
    ...
    for ep in range(epoch):
        ...
        for j in range(int(len(train_set) / batch_size)):
            ...
            noise = GEN(input)
            GEN_loss = torch.mean(torch.log(1.0 - DIS(noise))).to(device)
            GEN.zero_grad()
            GEN_loss.backward()
            optimizer_g.step()

            DIS_loss = -torch.mean(torch.log(DIS(func)) + torch.log(1.0 - DIS(noise.detach()))).to(device)
            DIS.zero_grad()
            DIS_loss.backward()
            optimizer_d.step()
        ...
        if ep % 10 == 9 :
            with torch.no_grad():
                ...
                draw.draw(ep, data,test_set, background , np.array(background_color.cpu().data), x_min, x_max, y_min, y_max, "gan_rsp")
                GEN.zero_grad()
                DIS.zero_grad()
...
if __name__ == "__main__":
    ...
    draw.merge(args.epoch / 10, "gan_rsp")